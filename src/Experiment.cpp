...
#pragma omp parallel for if (DO_PARALLEL)
  for (size_t g = 0; g < G; ++g) {
    Vector ps = model->features.prior.p.row(g).t();
    ...
    for (size_t s = 0; s < S; ++s) {
      for (size_t t = 0; t < T; ++t)
        rs[t] = model->features.prior.r(g, t) * features.prior.r(g, t)
                * theta(s, t) * spot(s);
...
Vector Experiment::sample_contributions_gene_spot(size_t g, size_t s,
  ...
  if (count > 0) {
    if (T == 1) {
      ...
      Vector r(T);
      for (size_t t = 0; t < T; ++t)
        r[t] = model->features.prior.r(g, t) * features.prior.r(g, t)
               * baseline_feature.prior.r(g) * theta(s, t) * spot(s);

      Vector p(T);
      for (size_t t = 0; t < T; ++t)
        p[t] = neg_odds_to_prob(model->features.prior.p(g, t));
      ...
      if (true) {
        for (size_t t = 0; t < T; ++t)
          cnts[t] = log(r[t] / model->features.prior.p(g, t));
      ...
      auto log_posterior_difference = [&](const Vector &v, size_t i, size_t j,
        ...
        const double r_i
            = model->features.prior.r(g, i) * features.prior.r(g, i);
        const double no_i = model->features.prior.p(g, i);
        const double prod_i = r_i * theta(s, i) * spot(s);

        const double r_j
            = model->features.prior.r(g, j) * features.prior.r(g, j);
        const double no_j = model->features.prior.p(g, j);
      ...
      for (size_t t = 0; t < T; ++t)
        z += mean_prob[t] = baseline_feature.prior.r(g) * features.prior.r(g, t)
                            * model->features.prior.r(g, t)
                            / model->features.prior.p(g, t) * theta(s, t);
      for (size_t t = 0; t < T; ++t)
        mean_prob[t] /= z;
...
#pragma omp parallel for if (DO_PARALLEL)
  for (size_t g = 0; g < G; ++g)
    for (size_t t = 0; t < T; ++t)
      explained(g, t) = model->features.prior.r(g, t)
                        * baseline_feature.prior.r(g)
                        / model->features.prior.p(g, t) * theta_t(t);
  return explained;
}
...
#pragma omp parallel for if (DO_PARALLEL)
  for (size_t g = 0; g < G; ++g)
    for (size_t t = 0; t < T; ++t)
      explained(g) += features.prior.r(g, t) * model->features.prior.r(g, t)
                      * baseline_feature.prior.r(g)
                      / model->features.prior.p(g, t) * theta_t(t);
  return explained;
};
...
Matrix Experiment::explained_spot_type() const {
  ...
  for (size_t t = 0; t < T; ++t) {
    Float x = 0;
    for (size_t g = 0; g < G; ++g)
      x += features.prior.r(g, t) * model->features.prior.r(g, t)
           * baseline_feature.prior.r(g) / model->features.prior.p(g, t);
    for (size_t s = 0; s < S; ++s)
      m(s, t) *= x * spot(s);
  }
  return m;
