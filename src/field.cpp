...
int main(int argc, char **argv) {
  ...
  size_t n = 2000;
  size_t num_fixed = 50;
  const size_t oversampling_factor = 10;
  // size_t num_fixed = 3;
  const double grid_scale = 10;
  const size_t small_n_iter = 1000000;
  // const size_t n_iter = 50;

  const size_t n_iter = 5000000000;
  double alpha = 1e-7;
  const double beta = 0.999999;
  const size_t report_interval = 100000;
  ...
  if (argc == 2) {
    ...
    double val_;
    while (ifs >> pt[0] >> pt[1] >> pt[2] >> val_) {
      pts.push_back(pt);
      val.push_back(val_);
      // cerr << "point: " << pt.t() << endl;
    }
    ...
    for (size_t i = 0; i < oversampling_factor * n; ++i) {
      ...
      p[2]
          = min_z
            + (max_z - min_z) * RandomDistribution::Uniform(EntropySource::rng);
      pts.push_back(p);
    }
